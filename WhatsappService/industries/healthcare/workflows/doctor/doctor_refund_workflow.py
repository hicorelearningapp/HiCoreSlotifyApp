...
class DoctorViewRefundsWorkflow(Workflow):
    def Initialize(self, session: ConversationSession):
        ...
        if not pending_refunds:
            return WorkflowResult.end_sequence(reply=Reply("text", "You have no pending refunds to process!"))
...
class DoctorProcessRefundWorkflow(Workflow):
    def Initialize(self, session: ConversationSession):
        ...
        if not appointment or appointment.get("RefundStatus") != "Pending":
            return WorkflowResult.end_sequence(reply=Reply("text", "This refund has already been processed or is invalid."))
        ...
        if message.InteractiveId == "CONFIRM_REFUND_SENT":
            ...
            try:
                ...
                if appointment:
                    ...
                    return WorkflowResult.end_sequence(reply=Reply("text", "Error processing refund."))
            except Exception as e:
                return WorkflowResult.end_sequence(reply=Reply("text", f"Error processing refund: {str(e)}"))
                
        elif message.InteractiveId == "CANCEL_REFUND_ACTION":
            return WorkflowResult.end_sequence(reply=Reply("text", "Refund processing aborted for now."))
