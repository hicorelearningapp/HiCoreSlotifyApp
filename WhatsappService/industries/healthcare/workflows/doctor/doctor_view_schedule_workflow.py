...
class DoctorViewScheduleWorkflow(Workflow):
    ...
    def Process(self, session: ConversationSession, message: Message):
        ...
        if step == "select_day":
            ...
            if message.InteractiveId == "SCHED_TODAY":
                ...
                if not unique_dates:
                    return WorkflowResult.end_sequence(reply=Reply("text", "You have no upcoming appointments beyond tomorrow."))
        ...
        # STEP 3: Handle Appointment Selection (for details)
            ...
            return WorkflowResult.end_sequence(reply=Reply("text", "Exiting schedule view. Type 'hi' to start over."))
    ...
    def _show_schedule(self, session, doctor_id, target_date):
        ...
        if not appointments:
            return WorkflowResult.end_sequence(reply=Reply("text", f"You have no appointments scheduled for {target_date.strftime('%b %d, %Y')}. 🎉\n\nType 'hi' to return to menu."))
    ...
    def _send_excel(self, session, doctor_id):
        try:
            ...
            return WorkflowResult.end_sequence(reply=Reply("text", "Excel export is not available (openpyxl is not installed)."))
            
        
        target_date_str = session.WorkflowData.get("target_date")
        if not target_date_str:
            return WorkflowResult.end_sequence(reply=Reply("text", "Error: Date not found."))
