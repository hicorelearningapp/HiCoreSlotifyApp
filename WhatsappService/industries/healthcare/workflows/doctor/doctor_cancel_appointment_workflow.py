...
class DoctorSelectAppointmentsToCancelWorkflow(Workflow):
    def Initialize(self, session: ConversationSession):
        ...
        if not appointments:
            return WorkflowResult.end_sequence(reply=Reply("text", "You don't have any upcoming appointments to cancel."))
...
class DoctorCancelAppointmentsWorkflow(Workflow):
    def Initialize(self, session: ConversationSession):
        ...
        if not target_cancel_ids:
            return WorkflowResult.end_sequence(reply=Reply("text", "No appointments selected. Cancellation aborted."))
    ...
    def Process(self, session: ConversationSession, message: Message):
        if message.InteractiveId == "CONFIRM_YES":
            ...
            return WorkflowResult.end_sequence(reply=Reply("text", "Cancellation aborted. The appointments remain on your schedule."))
