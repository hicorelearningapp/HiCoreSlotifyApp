...
from config import PUBLIC_BASE_URL, SERVER_BASE_URL
...
class GreetingMessageWorkflow(Workflow):
    def Initialize(self, session: ConversationSession) -> WorkflowResult:
        ...
        settings = HealthcareSequenceManager.get_settings(business_phone)
        
        welcome_message_override = settings.get("welcome_message_override")
        if welcome_message_override and not user_name:
            greeting = welcome_message_override

        if role != "admin":
            image_filename = settings.get("welcome_image_filename", "Welcome to HiCore Image English.jpeg")
            if not image_filename:
                return WorkflowResult.completed(
                    reply=Reply(message_type="text", text=greeting)
                )
            image_url = f"{SERVER_BASE_URL}/industries/healthcare/images/{urllib.parse.quote(image_filename)}"
            return WorkflowResult.completed(
