    ...
    def Process(self, session: ConversationSession, message: Message):
        if message.InteractiveId == "CONFIRM_NO":
            return WorkflowResult.completed(
                reply=Reply("text", "Booking process cancelled.")
            )
    ...
    def _finalize_booking(self, session: ConversationSession):
        try:
            ...
            if meeting_link:
                ...
                if email_to_use:
                    success_msg += f"📧 *(Join using this email: {email_to_use})*\n"
            elif doctor and doctor.get("ClinicAddress"):
                success_msg += f"\n\nClinic Address: {doctor.get('ClinicAddress')}"

            success_msg += "\n\nThank you for booking your appointment with HiCore!"
