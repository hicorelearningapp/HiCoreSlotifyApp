...
class CollectEmailWorkflow(Workflow):
    def Initialize(self, session: ConversationSession):
        ...
        return WorkflowResult.waiting(
            reply=Reply("text", "Please enter your new email address:")
        )

    def _send_confirmation_prompt(self, session: ConversationSession, email: str):
        prompt_text = f"Make Google Meet using this email ID: {email}?"
        return WorkflowResult.waiting(
            reply=Reply(
                message_type="buttons",
                text=prompt_text,
                options=[
                    {"id": "YES_EMAIL", "title": "Yes, use this"},
                    {"id": "NO_EMAIL", "title": "No, change it"},
                ],
            )
        )
    ...
    def Process(self, session: ConversationSession, message: Message):
        ...
        if step == "asking_email":
            ...
            if not re.match(email_pattern, email):
                return WorkflowResult.waiting(
                    reply=Reply("text", "That doesn't look like a valid email address. Please try again:")
                )
