...
class ProductRouter:
    def __init__(self):
        self.router = APIRouter(prefix="/products", tags=["Ecommerce Products"])
        self._add_routes()

    def _add_routes(self):
        self.router.add_api_route(
            "/{product_id}/product-workflow",
            self.get_product_workflow,
            methods=["GET"],
            summary="Get workflow .txt configuration for a product by product ID",
        )
        self.router.add_api_route(
        ...
        self.router.add_api_route(
            "/whatsapp-link",
            self.get_product_whatsapp_link,
            methods=["GET"],
            response_model=ProductWhatsAppLinkOut,
            summary="Generate WhatsApp chat link for a product by Reel link",
        )
        self.router.add_api_route(
            "",
            self.create_product,
    ...
    def get_product_workflow(self, product_id: str, db: Session = Depends(get_db)):
        """
